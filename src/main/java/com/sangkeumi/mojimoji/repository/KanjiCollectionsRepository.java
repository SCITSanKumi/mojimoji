...
public interface KanjiCollectionsRepository extends JpaRepository<KanjiCollection, Long> {
    ...
    // MySQL 8.0에서는 WITH 구문을 지원하므로 그대로 사용
    @Query(value = """
            WITH cat_total AS (
            ...
            SELECT
                (SELECT COUNT(*) FROM cat_total) AS total_category_count,
                (SELECT COUNT(*) FROM fully_collected) AS fully_collected_category_count
            """, nativeQuery = true)
    ...
    @Query("""
            SELECT new com.sangkeumi.mojimoji.dto.kanji.KanjiSearchResponse(
                ...
                COALESCE(kc.createdAt, '1970-01-01 00:00:00') AS firstCollectedAt
            )
            FROM Kanji k
    ...
    @Query(value = """
            SELECT kc.kanji_collection_id, kc.user_id, kc.bookmarked, kc.collected_count, kc.wrong_count, kc.created_at, kc.updated_at, k.*
            FROM Kanji_Collections kc
            INNER JOIN Kanjis k ON kc.kanji_id = k.kanji_id
            WHERE kc.user_id = :userId AND kc.wrong_count >= 1
