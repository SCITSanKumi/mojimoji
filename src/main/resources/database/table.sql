...
------------------------------------------------------------
-- Community_Replies 테이블: 커뮤니티 게시글 댓글 정보를 저장합니다.
------------------------------------------------------------
...
------------------------------------------------------------
-- Shared_Books 테이블: 공유된 책 정보를 관리합니다.
------------------------------------------------------------
CREATE TABLE Shared_Books (
    shared_book_id INT NOT NULL GENERATED BY DEFAULT AS IDENTITY, -- 공유된 책 고유 식별자
    book_id INT NOT NULL,                                          -- 공유되는 책 (Books 테이블)
    hit_count INT NOT NULL DEFAULT 0,                              -- 조회수
    gaechu INT NOT NULL DEFAULT 0,                                 -- 추천 수 (좋아요 수)
...
------------------------------------------------------------
-- Kanji_Collections 테이블: 개별 한자 수집 이벤트 기록
------------------------------------------------------------
CREATE TABLE Kanji_Collections (
    kanji_collection_id INT NOT NULL GENERATED BY DEFAULT AS IDENTITY, -- 개별 수집 이벤트 고유 식별자
    kanji_id INT NOT NULL,                                             -- 수집된 한자 (Kanjis 테이블)
    user_id INT NOT NULL,                                              -- 수집한 사용자 (Users 테이블)
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,            -- 수집 시각
