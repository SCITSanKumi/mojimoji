...
CREATE TABLE Kanjis (
    ...
    category VARCHAR(20),                                   -- 한자의 분류 (예: 기초, 중급, 고급 등)
    kanji VARCHAR(4) NOT NULL,                              -- 실제 한자 문자 (최대 4자)
    kor_onyomi VARCHAR(20),                                 -- 한자의 한국식 온음
    kor_kunyomi VARCHAR(20),                                -- 한자의 한국식 훈독
    jpn_onyomi VARCHAR(20),                                 -- 한자의 일본식 온음
    jpn_kunyomi VARCHAR(20),                                -- 한자의 일본식 훈독
...
CREATE TABLE Book_Lines (
    ...
    played_turns INT NOT NULL,                              -- 해당 줄이 플레이(사용)된 횟수 또는 턴 수
    is_ended BOOLEAN NOT NULL,                              -- 이 줄이 책의 종료 부분을 나타내는지 여부 (true이면 책 종료)
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 줄 생성 일시
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, -- 줄 마지막 수정 일시
    PRIMARY KEY (line_id),
